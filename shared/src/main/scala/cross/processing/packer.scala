...
object packer {
  ...
  def pack(list: List[Vec2d], padding: Vec2d): (Vec2d, List[Rec2d]) = {
    val (total, placements) = list
      .map(v => v + padding * 2)
      .sortBy(v => -(v.x * v.y))
      .foldLeft[(Rec2d, List[Rec2d])](Rec2d.Zero, Nil) { case ((totalArea, placed), current) =>
      val optimalRect = placed
        // find possible placements neat previously placed rectangles
        .flatMap { area => area.pointAt(Vec2d.TopRight) :: area.pointAt(Vec2d.BottomLeft) :: Nil }
    ...
    total.size -> placements.map(p => p.offsetBy(padding).resizeTo(p.size - padding * 2))
  }

  /** Produces tileset image and data from source images */
  def generateTileset(tileset: Tileset, workdir: String): Unit = {
    println("foo")
    tileset.images.foreach(i => println(i))
  }
}